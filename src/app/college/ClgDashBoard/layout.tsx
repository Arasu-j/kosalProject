...
export default function CollegeDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Sidebar */}
      <aside className={`w-64 bg-blue-900 text-white p-6 space-y-4 fixed inset-y-0 left-0 z-30 h-full transform transition-transform duration-300 ease-in-out ${
        isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        <h2 className="text-xl lg:text-2xl font-bold mb-6 mt-12 lg:mt-0">College Dashboard</h2>
        {navItems.map((item) => {
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 lg:p-8 bg-[#f4f8ff] lg:ml-64 min-h-screen overflow-y-auto">
        {children}
      </main>
    </div>
  )
